...
class MenuitemForm extends \Application\Form
{
    ...
    /**
     * Adds 'module_view_param' input. An input with possible paremeters for chosenView in chosenModule
     */
    public function addModuleViewParametersInput()
    {
        $module_view_params = $this->moduleManager->getModuleViewParams($this->chosenModule, $this->chosenModuleView);
        if (is_array($module_view_params))
            $this->addSelect('module_view_param', 'with parameter', $module_view_params);
        elseif (is_string($module_view_params))
            $this->addText('module_view_param', 'with parameter');
        else
            $this->addHidden('module_view_param');
    }
    ...
    public function issetChosenModule()
    {
        return isset($this->session->chosenModule);
    }
